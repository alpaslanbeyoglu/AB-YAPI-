...
export const CostDetailsTab: React.FC<CostDetailsTabProps> = ({
  ...
  const materialItems: MaterialTakeoffItem[] = [
    ...
    },
    {
      id: 'm3-a',
      category: 'kaba',
      name: 'Beton Döküm İşçiliği',
      unit: 'm³',
      quantity: results.concreteM3,
      unitPrice: (params.costKabaWork || 0) * 0.15,
      total: results.concreteM3 * (params.costKabaWork || 0) * 0.15 * (params.costMultiplier || 1),
      laborShare: 100,
    },
    {
      id: 'm3-b',
      ...
      unit: 'Ton',
      quantity: results.steelTon,
      unitPrice: (params.costKabaWork || 0) * 0.5,
      total: results.steelTon * (params.costKabaWork || 0) * 0.5 * (params.costMultiplier || 1),
      laborShare: 100,
    },
    {
      id: 'm3-c',
      ...
      unit: 'm³',
      quantity: totalArea,
      unitPrice: (params.costKabaWork || 0) * 0.6,
      total: totalArea * (params.costKabaWork || 0) * 0.6 * (params.costMultiplier || 1),
      laborShare: 100,
    },
    {
      id: 'm3-d',
      category: 'kaba',
      name: 'Tuğla Duvar Örme İşçiliği',
      unit: 'm²',
      quantity: totalArea * 0.3,
      unitPrice: (params.costKabaWork || 0) * 0.25,
      total: totalArea * 0.3 * (params.costKabaWork || 0) * 0.25 * (params.costMultiplier || 1),
      laborShare: 100,
    },
    // Tesisat & Sistemler
    {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in print:p-0">
      ...
      {/* 4. Interactive Commodity / Object Search Engine & Live Value Cards */}
      <div className={`p-5 rounded-2xl border ${isGray ? 'bg-slate-100 border-slate-300' : 'bg-white border-slate-200'} shadow-sm space-y-4`}>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-extrabold text-slate-800 flex items-center gap-2">
                Emtia, Nesne ve Malzeme Arama Motoru
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                  Canlı Değer Sorgulama
                </span>
              </h3>
              <p className="text-[11px] text-slate-500 mt-0.5">
                Aramak istediğiniz emtia (beton, demir, asansör, mutfak, PVC vb.) veya nesneyi yazın; sayfadaki güncel maliyet değeri, birim fiyatı ve metrajı anlık gelsin.
              </p>
            </div>
          </div>

          {/* Quick Clear if searched */}
          {searchQuery && (
            <button
              type="button"
        ...
        {/* Search Input Box & Quick Commodity Filter Tags */}
        <div className="space-y-2.5">
          <div className="relative">
            <Search className="w-4 h-4 text-indigo-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input
              type="text"
              placeholder="Emtia veya nesne adı yazın (Örn: Beton, Demir, Asansör, PVC, Mutfak, Kapı, Doğalgaz, Seramik, SGK, Noter...)"
              value={searchQuery}
        ...
        {/* Live Search Value Spotlight Cards when Query is Active */}
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center justify-between bg-indigo-50/70 border border-indigo-200 rounded-xl px-3.5 py-2">
              <span className="text-xs font-bold text-indigo-950 flex items-center gap-2">
                ...
                <span>
                  "{searchQuery}" için <strong>{filteredItems.length}</strong> emtia/nesne bulundu
                </span>
              </span>
              <span className="text-xs font-bold text-indigo-900 font-mono">
                Bulunanların Toplam Değeri:{' '}
                <span className="text-sm font-black text-indigo-700">
                  {sym}
                  {(
                    filteredItems.reduce((sum, item) => sum + item.total, 0) * rate
            ...
            {filteredItems.length === 0 ? (
              <div className="p-6 text-center bg-slate-50 rounded-xl border border-slate-200 text-slate-500 text-xs">
                "{searchQuery}" ile eşleşen bir malzeme veya emtia kaydı bulunamadı. Lütfen farklı bir arama terimi deneyin.
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {filteredItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="p-4 bg-white rounded-2xl border-2 border-indigo-200/90 shadow-sm hover:border-indigo-500 hover:shadow-md transition-all space-y-2.5 relative overflow-hidden"
                    >
                      ...
                      {/* Prominent Page Live Value Block */}
                      <div className="p-3 bg-gradient-to-br from-slate-900 to-indigo-950 text-white rounded-xl shadow-xs">
                        <span className="text-[9px] font-bold tracking-wider text-slate-300 uppercase block">
                          Sayfadaki Toplam Değeri
                        </span>
                        <div className="text-lg font-black text-amber-400 font-mono mt-0.5 flex items-baseline gap-1">
                          <span>{sym}{(item.total * rate).toLocaleString('tr-TR', { maximumFractionDigits: 0 })}</span>
                          {currency === 'TL' && (
                            <span className="text-[10px] text-slate-300 font-normal">
                              (${(item.total / (params.usdRate || 34.5)).toLocaleString('tr-TR', { maximumFractionDigits: 0 })})
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                        <div className="p-2 bg-slate-50 rounded-lg border border-slate-100">
                          <span className="text-[10px] text-slate-500 block font-medium">Birim Fiyat:</span>
                          <span className="font-bold text-slate-800 font-mono">
                            {sym}{(item.unitPrice * rate).toLocaleString('tr-TR', { maximumFractionDigits: 0 })} / {item.unit}
                          </span>
                        </div>
                        <div className="p-2 bg-slate-50 rounded-lg border border-slate-100">
                          <span className="text-[10px] text-slate-500 block font-medium">Hesaplanan Metraj:</span>
                          <span className="font-bold text-slate-800 font-mono">
      ...
      {/* 5. Large Interactive Metraj (Take-off) and Labor Ledger Table */}
      <div className={`p-5 rounded-2xl border ${isGray ? 'bg-slate-100 border-slate-300' : 'bg-white border-slate-200'} shadow-sm`}>
