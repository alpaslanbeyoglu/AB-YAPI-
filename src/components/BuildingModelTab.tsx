                ...
                <div className="p-5 pt-0 space-y-4 border-t border-slate-100">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                      {(['1+1', '2+1', '3+1', '4+1', '5+1'] as RoomType[]).map((type) => (
                        <button
                          key={type}
                          type="button"
            ...
            <div className={`border rounded-3xl overflow-hidden ${cardBg}`}>
              ...
              {!collapsedSections.shafts && (
                <div className="p-5 pt-0 space-y-4 border-t border-slate-100">
                  ...
                  {/* Elevator Count */}
                  <div className="space-y-1.5">
                    <label className={`text-xs font-semibold block ${textTitle}`}>Asansör Sayısı:</label>
                    <div className="grid grid-cols-2 gap-2">
                      {[1, 2].map((cnt) => (
                        <button
                          key={cnt}
                          type="button"
                          onClick={() => updateParams({ elevatorCount: cnt })}
                          className={`py-1.5 text-center rounded-xl text-xs font-semibold border transition-all ${
                            (modelParams.elevatorCount ?? 1) === cnt
                              ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm'
                              : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'
                          }`}
                        >
